    ...
    FileSpec
        ...
        .addType(
            TypeSpec.classBuilder(kGraphName)
                ...
                .addType(typeSpec = getCompanionBuilder(graphClass, kGraphClass)).build()
        ).build().writeTo(codeGenerator = codegen.generator, aggregating = false, originatingKSFiles = codegen.files)
}

private fun getCompanionBuilder(graphClass: TypeName, kGraphClass: TypeName): TypeSpec {
    val from = FunSpec.builder("from").addParameter(GRAPH_PARAM, graphClass).returns(kGraphClass).addStatement("return %T($GRAPH_PARAM)", kGraphClass).build()
